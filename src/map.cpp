...
bool writeVisibilityData(const char *fileName)
{
	...
	{
		debug(LOG_ERROR, "writeVisibilityData: could not write header to %s; PHYSFS error: %s", fileName, PHYSFS_getLastErrorCode());
	...
	for (unsigned plane = 0; plane < planes; ++plane)
	{
		for (i = 0; i < mapWidth * mapHeight; ++i)
		{
			...
			{
				debug(LOG_ERROR, "writeVisibilityData: could not write to %s; PHYSFS error: %s", fileName, PHYSFS_getLastErrorCode());
				PHYSFS_close(fileHandle);
				return false;
			}
		}
...
bool readVisibilityData(const char *fileName)
{
	...
	{
		debug(LOG_ERROR, "readVisibilityData: error while reading header from file: %s", PHYSFS_getLastErrorCode());
		PHYSFS_close(fileHandle);
		return false;
	}
	...
	for (unsigned plane = 0; plane < planes; ++plane)
	{
		for (i = 0; i < mapWidth * mapHeight; i++)
		{
			...
			{
				debug(LOG_ERROR, "readVisibilityData: could not read from %s; PHYSFS error: %s", fileName, PHYSFS_getLastErrorCode());
				PHYSFS_close(fileHandle);
				return false;
			}
