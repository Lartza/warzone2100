...
static bool CheckInMap(const char *archive, const char *mountpoint, const char *lookin)
{
	...
	if (!PHYSFS_mount(archive, mountpoint, PHYSFS_APPEND))
	{
		...
		debug(LOG_FATAL, "Could not mount %s, because: %s. Please delete the file, and run the game again. Game will now exit.", archive, PHYSFS_getLastErrorCode());
		exit(-1);
	}
	...
	{
		debug(LOG_ERROR, "Could not unmount %s, %s", archive, PHYSFS_getLastErrorCode());
	}
	return mapmod;
}
...
bool buildMapList()
{
	...
	for (auto &realFileName : realFileNames)
	{
		...
		{
			debug(LOG_ERROR, "Could not unmount %s, %s", realFilePathAndName.c_str(), PHYSFS_getLastErrorCode());
		}
